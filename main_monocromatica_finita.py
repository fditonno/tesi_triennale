#in questo file simulo il segnale generato da un'onda gravitazionale ad ampiezza costante e frequenza pari a quella di risonanza con una durata limitata
...
class GW: #onda gravitazionale
    def __init__(self):
        ...
        self.inizio = [-1, 5e-6, 25e-6, 100] #tempi di inizio dei segnali monocromatici (s)
        self.pulsazione = [0, 2*np.pi*5e6, 0] #pulsazioni (Hz)
        self.ampiezza = [0, 1e-21, 0] #ampiezze (strain)
...
#grafico dell'onda gravitazionale
ax[0].set_ylabel("h [strain]")
ax[0].grid(True)
...
t_inizio = t[-1] #tempo di inizio (s)
B_0 = B[-1] #B(0) (m)
dB_0 = dB[-1] #B'(0) (m/s)
...
#durata della decrescita
i=0
massimo = max(ampiezze_B)
while ampiezze_B[i]>massimo*0.01:
    i=i+1
print(tempi_B[i]) #tempo in cui si scende sotto l'1% dell'ampiezza massima
